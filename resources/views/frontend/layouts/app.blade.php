...
<body class="text-left">
    ...
    <script type="text/javascript">
        ...
        call = normalizeCallPayload(call);
        clearTimeout(window.currentCallTimer);
        window.currentCallTimer = setTimeout(function () {
            var current = normalizeCallPayload(window.currentCallState || {});
            if (!current || !call || current.id !== call.id) { return; }
            var status = String(current.status || call.status || '').toLowerCase();
...
(function (window, $) {
    ...
    function normalizeCallPayload(call) {
        if (call && call.call && !call.id) {
            call = call.call;
        }
        return call || {};
    }

    function ensureVideoLayout() {
        var $area = $('#global-call-video-area');
        if (!$area.length) { return; }
        $area.removeClass('d-none');
        if (!$('#global-remote-video-area').length) {
            $area.append('<div id="global-remote-video-area" class="global-remote-video-screen"></div>');
        }
        if (!$('#global-local-video-preview').length) {
            $area.append('<div id="global-local-video-preview" class="global-local-video-preview"></div>');
        }
    }

    function replayLocalVideo() {
        if (window.currentCallVideoTrack && $('#global-local-video-preview').length) {
            try { window.currentCallVideoTrack.play('global-local-video-preview'); } catch (e) { console.warn('[global call] local video play failed', e); }
        }
    }
    ...
    window.showOutgoingCallModal = function (call) {
        ...
        var peer = getCallCounterpart(call) || {};
        ...
        try {
            ...
            if (!isCaller || ['accepted', 'connected'].indexOf(String(call.status || '').toLowerCase()) !== -1) {
                $.ajax({
                    ...
                    success: function (response) {
                        if (response && response.success && response.data && response.data.call && typeof window.openActiveCallScreen === 'function') {
                            window.openActiveCallScreen(response.data.call);
                        }
                    }
                });
            }
...
<script type="text/javascript">
(function (window) {
    // Final call-timeout guard. A ringing timeout must never terminate an accepted/connected call.
    window.startIncomingCallCountdown = function (call) {
        clearTimeout(window.currentCallTimer);
        window.currentCallTimer = setTimeout(function () {
            var current = window.currentCallState || {};
            var currentId = current && current.id ? parseInt(current.id, 10) : null;
            var callId = call && call.id ? parseInt(call.id, 10) : null;
            if (!currentId || !callId || currentId !== callId) {
                window.currentCallTimer = null;
                return;
            }
    ...
    var originalOpenActiveCallScreen = window.openActiveCallScreen;
    if (typeof originalOpenActiveCallScreen === 'function') {
        window.openActiveCallScreen = function (call) {
            clearTimeout(window.currentCallTimer);
            window.currentCallTimer = null;
            ...
            if (window.currentCallState && (!call || window.currentCallState.id == call.id)) {
                window.currentCallState.status = 'connected';
            }
            return originalOpenActiveCallScreen.apply(this, arguments);
        };
    }
})(window);
</script>
...
<script type="text/javascript">
(function (window, $) {
    function callObj(event) {
        if (!event) { return {}; }
        if (event.call) { return event.call; }
        if (event.data && event.data.call) { return event.data.call; }
        return event;
    }

    function callPeer(call) {
        var userId = parseInt(window.currentUserId || @json(Auth::id()), 10);
        var caller = call && call.caller ? call.caller : {};
        var receiver = call && call.receiver ? call.receiver : {};
        var callerId = caller.id ? parseInt(caller.id, 10) : null;
        var receiverId = receiver.id ? parseInt(receiver.id, 10) : null;

        if (callerId && callerId === userId) { return receiver; }
        if (receiverId && receiverId === userId) { return caller; }
        return call.peer || receiver || caller || {};
    }
    ...
    var previousHandleCallSignal = window.handleCallSignal;
    window.handleCallSignal = function (status, event) {
        var call = callObj(event);
        if (!call || !call.id) {
            if (typeof previousHandleCallSignal === 'function') {
                return previousHandleCallSignal(status, event);
            }
            return;
        }
        ...
        if (['rejected', 'cancelled', 'busy', 'ended', 'missed', 'failed'].indexOf(status) !== -1) {
            $('#global-call-action-modal').modal('hide');
            if (typeof window.stopCallSession === 'function') {
                window.stopCallSession();
            }
        }
    };
