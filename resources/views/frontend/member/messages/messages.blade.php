...
(function (window, $) {
    ...
    function ensureVideoLayout() {
        var $area = $('#call-video-area');
        if (!$area.length) { return; }
        $area.removeClass('d-none');
        if (!$('#remote-video-area').length) {
            $area.append('<div id="remote-video-area" class="remote-video-screen"></div>');
        }
        if (!$('#local-video-preview').length) {
            $area.append('<div id="local-video-preview" class="local-video-preview"></div>');
        }
    }

    function replayLocalVideo() {
        if (window.currentCallVideoTrack && $('#local-video-preview').length) {
            try { window.currentCallVideoTrack.play('local-video-preview'); } catch (e) { console.warn('[chat call] local video play failed', e); }
        }
    }
    ...
    window.startCall = function (type) {
        ...
        callRequest('{{ route('chat.call.start') }}', {
            ...
            window.currentCallState = normalizeCallPayload(response.data.call);
            window.currentCallState.rtc = response.data.rtc;
    ...
    (function (window, $) {
        if (!window.__hamqadamChatCallUiTweaked) {
            ...
            window.showOutgoingCallModal = function (call) {
                var peer = (typeof getCallCounterpart === 'function' ? getCallCounterpart(call) : null) || {};
                window.currentCallPeer = peer;
