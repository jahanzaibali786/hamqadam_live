...
@section('panel_content')
    <div class="card">
        ...
        <div class="card-body">
            @if(empty($horoscopeFilled))
                <div class="text-center py-5">
                    <div class="mb-3">
                        <i class="las la-star-of-life la-3x text-primary"></i>
                    </div>
                    <h5 class="mb-2">{{ translate('Complete your horoscope details first') }}</h5>
                    <p class="mb-4 text-muted">
                        {{ translate('We could not find any horoscope information in your profile yet. Add your horoscope details to unlock matched profiles.') }}
                    </p>
                    <a href="{{ route('profile_settings') }}#astronomic_information"
                       class="btn btn-primary btn-sm">
                        {{ translate('Fill Horoscope detail') }}
                    </a>
                </div>
            @else
                @if(isset($matchedProfiles) && $matchedProfiles->count() > 0)
                ...
                @else
                    <div class="text-center py-5">
                        <i class="las la-search-minus la-3x text-muted mb-3"></i>
                        <h5 class="mb-2">{{ translate('No horoscope matched profiles found yet') }}</h5>
                        <p class="mb-0 text-muted">{{ translate('Try refreshing later after your horoscope profile is completed and matching data is available.') }}</p>
                    </div>
                @endif
            @endif
        </div>
