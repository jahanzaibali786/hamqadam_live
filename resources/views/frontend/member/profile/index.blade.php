...
@section('panel_content')
                    ...
                    <div class="col-md-10">
                        <div class="input-group mb-3">
                          ...
                          <div class="input-group-append">
                             <button type="button" class="btn btn-outline-secondary new-email-verification" disabled>
    ...
    @include('frontend.member.profile.registration_details')

    <!-- Present Address -->
    @php
        $default_country_id    = \App\Models\Country::where('name', 'Pakistan')->value('id');
        $present_address      = \App\Models\Address::where('type','present')->where('user_id',$member->id)->first();
        $present_country_id   = $present_address->country_id ?? $default_country_id ?? "";
    ...
    @php
        $default_religion_id  = \App\Models\Religion::where('name', 'Islam')->value('id');
        $member_religion_id   =  $member->spiritual_backgrounds->religion_id ?? $default_religion_id ?? "";
    ...
    @php
        ...
        $permanent_country_id   = $permanent_address->country_id ?? $default_country_id ?? "";
    ...
    <!-- Partner Expectation -->
    @php
        $partner_religion_id   = $member->partner_expectations->religion_id ?? $default_religion_id ?? "";
        $partner_caste_id      = $member->partner_expectations->caste_id ?? "";
        $partner_sub_caste_id  = $member->partner_expectations->sub_caste_id ?? "";
        $partner_country_id    = $member->partner_expectations->preferred_country_id ?? $default_country_id ?? "";
...
<script type="text/javascript">
    ...
    function mark_required_profile_fields() {
        $('.card form').find('input[required], select[required], textarea[required]').each(function () {
            var field = $(this);
            var id = field.attr('id');
            var label = id ? $('label[for="' + id + '"]').first() : $();

            if (!label.length) {
                label = field.closest('.form-group, .col-md-6, .col-md-10').find('label').first();
            }

            var hasRequiredMark = label.find('.required-asterisk').length ||
                label.find('.text-danger').filter(function () {
                    return $(this).text().trim() === '*';
                }).length;

            if (label.length && !hasRequiredMark) {
    ...
    var verifyBtn = $('.new-email-verification');
