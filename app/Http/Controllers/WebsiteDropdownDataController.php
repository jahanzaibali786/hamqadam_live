...
class WebsiteDropdownDataController extends Controller
{
    private array $groups;
    private array $items;
    private array $slugTypes = [
        'sect_mains',
        'school_of_thoughts',
        'traditions',
        'education_levels',
    ...
    public function index(Request $request)
    {
        ...
        foreach ($groupItems as $type => $config) {
            $searches[$type] = $request->get('search_'.$type);
            $records[$type] = $this->queryForType($type, $searches[$type])->paginate(10, ['*'], $type.'_page')->appends($request->query());
        }

        return view('admin.website_settings.dropdown_data.index', compact('groups', 'activeTab', 'groupItems', 'records', 'searches', 'lookups'));
    }
    ...
    public function store(Request $request)
    {
        ...
        DB::transaction(function () use ($request, $type, $config) {
            $modelClass = $config['model'];
            $record = new $modelClass();
            $this->fillRecord($type, $record, $request);
            $record->save();
        });
    ...
    public function update(Request $request, string $type, string $id)
    {
        ...
        DB::transaction(function () use ($request, $type, $record) {
            $this->fillRecord($type, $record, $request);
            $record->save();
        });
    ...
    private function buildItems(): array
    {
        return [
            ...
            'countries' => [
                ...
                'model' => Country::class,
                'rules' => ['name' => ['required', 'max:255'], 'status' => ['nullable', 'in:0,1']],
                'messages' => [
                ...
                'columns' => [
                    ...
                    ['label' => translate('Status'), 'value' => fn ($record) => ! empty($record->status) ? translate('Active') : translate('Inactive')],
                ],
                'searchable' => ['name'],
            ...
            'states' => [
                ...
                'model' => State::class,
                'rules' => ['country_id' => ['required', 'integer'], 'name' => ['required', 'max:255']],
                'messages' => [
            ...
            'cities' => [
                ...
                'model' => City::class,
                'rules' => ['country_id' => ['required', 'integer'], 'state_id' => ['required', 'integer'], 'name' => ['required', 'max:255']],
                'messages' => [
                    'country_id.required' => translate('Country is required'),
            ...
            'castes' => [
                ...
                'model' => Caste::class,
                'rules' => ['religion_id' => ['required', 'integer'], 'name' => ['required', 'max:255']],
                'messages' => [
            ...
            'sub_castes' => [
                ...
                'rules' => ['caste_id' => ['required', 'integer'], 'name' => ['required', 'max:255']],
                'messages' => [
            ...
            'sect_mains' => [
                ...
                'columns' => [
                    ['label' => translate('Religion'), 'value' => fn ($record) => optional($record->religion)->name],
                    ['label' => translate('Name'), 'value' => fn ($record) => $record->name],
                    ['label' => translate('Status'), 'value' => fn ($record) => ! empty($record->is_active) ? translate('Active') : translate('Inactive')],
                ],
                'relations' => ['religion'],
                'searchable' => ['name'],
            ...
            'school_of_thoughts' => [
                ...
                'rules' => ['sect_main_id' => ['required', 'integer'], 'name' => ['required', 'max:255']],
                'messages' => [
            ...
            'traditions' => [
                ...
                'rules' => ['school_of_thought_id' => ['required', 'integer'], 'name' => ['required', 'max:255']],
                'messages' => [
            ...
            'degrees' => [
                ...
                'model' => Degree::class,
                'rules' => ['education_level_id' => ['required', 'integer'], 'name' => ['required', 'max:255'], 'category' => ['nullable', 'max:255'], 'sort_order' => ['nullable', 'integer'], 'is_active' => ['nullable', 'in:0,1']],
                'messages' => [
            ...
            'fields_of_study' => [
                ...
                'rules' => ['degree_id' => ['required', 'integer'], 'name' => ['required', 'max:255'], 'sort_order' => ['nullable', 'integer'], 'is_active' => ['nullable', 'in:0,1']],
                'messages' => [
            ...
            'institutions' => [
                ...
                'rules' => ['country_id' => ['required', 'integer'], 'state_id' => ['required', 'integer'], 'city_id' => ['required', 'integer'], 'name' => ['required', 'max:255'], 'type' => ['nullable', 'max:100'], 'description' => ['nullable', 'max:1000'], 'website' => ['nullable', 'max:255'], 'sort_order' => ['nullable', 'integer'], 'is_active' => ['nullable', 'in:0,1']],
                'messages' => [
                    'country_id.required' => translate('Country is required'),
                    'state_id.required' => translate('State is required'),
                ...
                'fields' => [
                    ...
                    ['name' => 'name', 'label' => translate('Name'), 'type' => 'text', 'required' => true],
                    ['name' => 'type', 'label' => translate('Type'), 'type' => 'select', 'options' => ['University' => 'University', 'College' => 'College', 'Institute' => 'Institute', 'School' => 'School', 'Madarsa' => 'Madarsa', 'Other' => 'Other']],
                ...
                'columns' => [
                    ...
                    ['label' => translate('Location'), 'value' => fn ($record) => trim(implode(', ', array_filter([optional($record->city)->name, optional($record->state)->name, optional($record->country)->name])))],
                    ['label' => translate('Status'), 'value' => fn ($record) => ! empty($record->is_active) ? translate('Active') : translate('Inactive')],
                ],
            ...
            'professions' => [
                ...
                'rules' => ['profession_category_id' => ['required', 'integer'], 'name' => ['required', 'max:255'], 'sort_order' => ['nullable', 'integer'], 'is_active' => ['nullable', 'in:0,1']],
                'messages' => [
            ...
            'annual_salary_ranges' => [
                ...
                'rules' => ['min_salary' => ['required', 'numeric'], 'max_salary' => ['required', 'numeric']],
                'messages' => [
    ...
    private function fillRecord(string $type, object $record, Request $request): void
    {
        if (in_array($type, $this->slugTypes, true) && $request->filled('name')) {
    ...
    private function cascadeDelete(string $type, object $record): void
    {
        switch ($type) {
            ...
            case 'religions':
                ...
                foreach (SectMain::where('religion_id', $record->id)->with(['schoolsOfThoughts.traditions'])->get() as $sectMain) {
