...
class PackageController extends Controller
{
    ...
    public function free_package_activate(Request $request)
    {
        $user = auth()->user();
        $package_payment = RegistrationReward::applyBasicPackage($user->loadMissing('member'), 'free_package_activate');

        // Package Payment Store Notification for member
        if ($package_payment) {
            try {
                $notify_type = 'free_package_activate';
                $id = unique_notify_id();
                $notify_by = $user->id;
                $info_id = $package_payment->id;
                $message = $user->first_name . ' ' . $user->last_name . translate('has been activated  free package. Payment Code: ') . $package_payment->payment_code;
        ...
        return response()->json(['result' => true, 'message' => translate("Package Successfully Activated")]);
