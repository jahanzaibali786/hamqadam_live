...
class HomeController extends Controller
{
    ...
    public function home_premium_members()
    {
        ...
        if ($token && $user->user_type == 'member') {
            ...
            if ($gender) {
                $member_ids = Member::where('gender', '!=', $gender)->pluck('user_id')->toArray();
    ...
    public function home()
    {
        ...
        if (auth()->user() && auth()->user()->user_type == 'member') {
            ...
            $gender = auth()->user()->member->gender ?? null;
            if ($gender) {
                $member_ids = Member::where('gender', '!=', $gender)->pluck('user_id')->toArray();
                $members = $members->whereIn('id', $member_ids);
            }
    ...
    public function member_dashboard()
    {
        ...
        $data['coin_balance'] = (int) $data['remaining_interest'];
        ...
        $current_package_info = array(
            'package_id' => $user->member->package->id ?? null,
            'package_name' => $user->member->package->name ?? null,
            'package_expiry' => $user->member->package_validity ? (strtotime($user->member->package_validity) < time() ? translate('Expired') : date('d.m.Y', strtotime($user->member->package_validity))) : null,
        );
